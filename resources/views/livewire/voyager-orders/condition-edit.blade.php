<div>
    ...
    <?php $conditions=Cart::session('backend')->getConditions();?>
    @if (!is_null($conditions))
        @foreach($conditions as $condition)
            @if ($condition->getType()!='coupon')
                <li><strong>{{$condition->getName()}}: </strong><strong>{{two_decimal($condition->getCalculatedValue(Cart::session('backend')->getSubtotal()))}}</strong>(<a href="#" wire:click.prevent="removeCondition('{{$condition->getType()}}')">Remove</a>)</li>
            @endif
        @endforeach
    @else 
